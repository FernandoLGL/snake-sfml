...
int main(int argc, char** argv)
{
  ...
  while (window.isOpen())
  {
    ...
    if (snake.isDead() || snake.getHead().getGlobalBounds().intersects(game.mLeftBorder.getGlobalBounds())
    ...
    // I'm also drawing it after checking if the snake is dead, which implies that the score won't be printed on the
    // screen on game over
    window.draw(scoreText);
