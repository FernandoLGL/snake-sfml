...
void Food::spawn()
{
  ...
  int randomX  = (((randomNumber % windowWidth) > windowWidth / 2) ? ((randomNumber % windowWidth) - 2 * borderSize) :
                                                                    ((randomNumber % windowWidth) + 2 * borderSize));
  randomNumber = std::rand();
  int randomY = (((randomNumber % windowHeight) > windowHeight / 2) ? ((randomNumber % windowHeight) - 2 * borderSize) :
                                                                      ((randomNumber % windowHeight) + 2 * borderSize));
